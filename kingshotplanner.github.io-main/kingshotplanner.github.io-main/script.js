class TerritoryPlanner {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('export-text').addEventListener('click', () => this.showExportModal());
        document.getElementById('import-text').addEventListener('click', () => this.showImportModal());
    ...
    updateCanvasSize() {
        ...
        // Update the canvas container to show fixed viewport with scrolling
        if (this.canvasContainer) {
            //const maxViewportWidth = Math.min(800, window.innerWidth - 320); // Account for toolbar
            //const maxViewportHeight = Math.min(600, window.innerHeight - 200); // Account for header
            let maxViewportWidth, maxViewportHeight;

            if (window.innerWidth <= 768) {
                // Mobile: use almost full screen
                maxViewportWidth = window.innerWidth - 20;
                maxViewportHeight = window.innerHeight - 220;
            } else {
                // Desktop
                maxViewportWidth = Math.min(800, window.innerWidth - 320);
                maxViewportHeight = Math.min(600, window.innerHeight - 200);
            }
            
            this.canvasContainer.style.width = Math.min(this.canvas.width, maxViewportWidth) + 'px';
            this.canvasContainer.style.height = Math.min(this.canvas.height, maxViewportHeight) + 'px';
    ...
    centerView() {
        if (this.canvasContainer) {
            const containerWidth = parseInt(this.canvasContainer.style.width);
            const containerHeight = parseInt(this.canvasContainer.style.height);
    ...
    importFromText(text) {
        try {
            ...
            for (const itemData of data.items) {
                const item = {
                    type: this.getTypeFromCode(itemData.t),
                    x: itemData.x,
                    y: itemData.y,
    ...
    showExportModal() {
        const exportText = this.exportToText();
    ...
    showSamplesModal() {
        ...
        if (typeof sampleLayouts !== 'undefined') {
